...
let operate_app_list = (
    ...
    final_price: number,
    pct_price: number,
    bundled_is_count: number,
    price_label: string,
    review: number,
    review_label: string,
    steam_deck_support: boolean,
    china: boolean,
    operate_date: Date = new Date() // 异步访问，并发写入
): Promise<void> => db.prepareSync(`
    INSERT INTO main.games (uuid, name, img, imgsrc, platform, release_date, original_price, final_price, pct_price,
...
if (!ISONLINE) analyze_dom(DOM.load(fs.readFileSync(OFFLINE_HTML, "utf8")))
else {
    let page = 1;
    ...
    while (page++ * 100 < data.total_count)
        analyze_dom(DOM.load((
            await fetch(`${URL_CHINA}&count=${PARAMETER_STEP}&page=${page}`).then(res => res.json()).catch(err => console.error(err))
        ).results_html));
}
...
function analyze_dom($: DOM.CheerioAPI): void {
    for (let el of $("a.search_result_row")) {
        ...
        for (let el of $(".search_name > div > .platform_img")) {
            switch ($(el).attr("class")!.split(' ')[1]) { // not undefined
                case "win": platform |= 0b10; break;
                case "music": platform |= 0b01; break;
            }
        }
        ...
        if ($review.length > 0) { // 没有错误处理！注意！容易恐慌 please be panic!
            ...
            switch ($review.attr("class")!.split(' ')[1]) {
                case "positive": review = 1; break;
                case "mixed": review = 0; break;
                case "negative": review = -1; break;
                default: review = -2;
            }
        }
        ...
        if (final_price > 65535 || origin_price > 65535) {
            ...
            if (final_price > 65535) final_price = 65535;
            if (origin_price > 65535) origin_price = 65535;
        }
...
// TODO : 下面是更多功能的安排
// 多文件安排、导入老版本兼容的json包、写下全局steam的api访问、单个包的fetch申请、单个表的外键控制
// 预计在25日完成
